...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // }
    for(int t = 0; t<N; t++) {
      fg[0] += 4500*CppAD::pow(vars[cte_start + t] + 5, 2) ;
      fg[0] += 6000*CppAD::pow(vars[epsi_start + t] + 1.2, 2) ;
      fg[0] += 170.0*CppAD::pow(vars[v_start + t] - ref_v, 2) ;
    }

    // Minimize the use of actuators
    for(int t = 0; t<N - 1; t++) {
      fg[0] += 5*CppAD::pow(vars[delta_start + t], 2) ;
      fg[0] += 20000*CppAD::pow(vars[a_start + t], 2) ;      
    }
    ...
    for(int t = 0; t<N - 2; t++) {
      ...
      fg[0] += 95000*CppAD::pow(vars[delta_start + t] * vars[v_start+t], 2);
    }
